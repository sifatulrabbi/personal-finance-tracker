...
RUN go test ./... && CGO_ENABLED=0 go build -trimpath -o /out/simply-finance ./cmd/simply-finance

FROM alpine:3.23
RUN addgroup -g 10001 finance && adduser -D -u 10001 -G finance finance && mkdir -p /data && chown finance:finance /data
WORKDIR /app
COPY --from=backend /out/simply-finance /app/
